...
void Slave_Reset (void){
	...
	NumCreated = 0 ;
	NumCreated += OS_AddThread(Thread_CAN_Init_Receive, 128, 2);
...
/********************* CAN_Init_Receive ***********************/
void Thread_CAN_Init_Receive (void){
 while(r_init_num == 0){
...
void PThread_CAN_Send (void){
	...
	if(start_sending && ( snum<(arr_size/8) )){
		for(int i=0; i<8; i++){
	...
	if(snum == (arr_size/8) ) start_sending=0;
}
