...
void Master_Reset (void){
	...
	NumCreated += OS_AddThread(Thread_Random_Input, 128, 2); // calls CAN_Send (priority 1)
	NumCreated += OS_AddThread(Thread_CAN_Receive, 128, 3);
	NumCreated += OS_AddThread(Thread_Print_Result, 128, 4);
...
void PThread_CAN_Send(void){
	
	if(start_sending && snum ==0){
			UART_OutString(" Start measuring time "); UART_OutCRLF();
			start_time = OS_usTime();  
	}
	if(start_sending && (snum < arr_size/8) && (ready_send > snum)){
...
//			UART_OutUDec(sdata[i]); UART_OutCRLF();
	...
	if((snum == arr_size) && start_sending) start_sending = 0;
...
uint32_t rnum=0;
/********************* CAN_Receive ***********************/
void Thread_CAN_Receive (void){
 while(rnum < (arr_size/8)){
		PF2 =0x04;
    if(CAN0_GetMailNonBlock(rdata)){
			
		UART_OutString(" Receiving numbers.... "); UART_OutUDec(rnum); UART_OutCRLF();
			for(int i=0; i<8; i++){
...
//				UART_OutUDec(rdata[i]); UART_OutCRLF();
		...
		//*****done measuring time*******//	
	if(rnum == arr_size/8){
		UART_OutString(" Done measuring time"); UART_OutCRLF();
		stop_time	= OS_usTime();
	}
...
//*****Print result*******//	
	 NumCreated += OS_AddThread(Thread_Print_Result, 128, 1);
